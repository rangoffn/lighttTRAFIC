...
func _ready(): 
	...
	if not Musicmenu.get_node("tolk").playing:
	...
	timer = get_tree().create_timer(55,false,false)
	randomize()
	schedule_next_spawn()
...
func _process(delta):
	...
	if bus_out == true:
		if Global.bus228 == true:
			$stilltime2.text = str(int(timer.get_time_left()))
			$stilltime.text = " "
		else: 
			$stilltime2.text = " "
			...
			if timeout_bus == true:
				...
				$stilltime2.text = "Не прибыл вовремя!"
			else: 
				$stilltime.text = " "
				$stilltime2.text = " "
...
func _on_bus_rotate_2_body_entered(body):
	var timer1 = get_tree().create_timer(0.5)
	if body.is_in_group("bus"):
		await timer1.timeout
		object.rotate(1.57)
